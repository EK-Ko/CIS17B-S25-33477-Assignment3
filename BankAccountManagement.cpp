...
class OverdrawException : public std::runtime_error {
    public:
        OverdrawException() : std::runtime_error("Withdrawal amount exceeds available balance.") {}
    };
...
class InvalidAccountOperationException : public std::runtime_error {
    public:
        InvalidAccountOperationException() : std::runtime_error("Transaction not allowed: Account is closed.") {}
    };
...
public:
    ...
    void withdraw(double amount) {
        ...
        if (amount < 0) throw std::invalid_argument("Cannot withdraw a negative amount.");
        balance -= amount;
    }
...
int main() {
    try {
        ...
        std::cerr << "Deposit Error: " << e.what() << "\n";
    }
    catch (const OverdrawException& e) {
        std::cerr << "Withdrawal Error: " << e.what() << "\n";
    }
    catch (const InvalidAccountOperationException& e) {
        std::cerr << "Invalid Operation: " << e.what() << "\n";
    } 
